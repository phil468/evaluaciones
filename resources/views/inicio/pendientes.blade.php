...
@section('js')
    <script>
        $(function() {
            ...
            const RUTA_EVALUACION =
                "{{ route('evaluacion.show', ['tipo_de_evaluacion_id' => ':tipo', 'id' => ':id']) }}";
            const RUTA_PLANES_MEJORA =
                "{{ route('planes-de-mejora', ['dashboard' => 'dashboard', 'empleado_id' => ':empleado_id']) }}";
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.secciones_activas.competencias) {
                            actualizarSeccion('competencias', response.evaluaciones_competencias,
                                response.estadisticas_competencias, 1);
                        }

                        if (response.secciones_activas.objetivos) {
                            actualizarSeccion('objetivos', response.evaluaciones_objetivos, response
                                .estadisticas_objetivos, 2);
                        }

                        if (response.secciones_activas.planes) {
                            actualizarPlanesMejora(response.planes_mejora, response
                            .estadisticas_planes);
                        }
                    },
                    error: function(xhr) {
            ...
            function generarTarjetaPlanMejora(plan) {
                console.log(plan);
                const nombreCompleto =
                    `${plan.empleado.nombres || ''} ${plan.empleado.apellido_paterno || ''} ${plan.empleado.apellido_materno || ''}`
                    .trim();
                ...
                if (!plan.habilitado) {
                    ...
                    estadoBadge = 'badge-warning';
                    estadoBtn = 'btn-vanguard';
                    estadoTexto = plan.plan_de_mejora.primera_fase_activa ? 'Ingresar Feedback y PMI' :
                        'Ingresar Cumplimiento';
                }

                // Calcular resumen de estados de aprobación
                let resumenHTML = '';
                if (plan.plan_de_mejora.primera_fase_activa && plan.planes_de_mejora && plan.planes_de_mejora
                    .length > 0) {
                    const totalPlanes = plan.planes_de_mejora.length;
                    const pendientes = plan.planes_de_mejora.filter(p => p.estado_aprobacion === 'pendiente')
                    .length;
                    const noValidados = plan.planes_de_mejora.filter(p => p.estado_aprobacion === 'no_validado')
                        .length;
                    const validados = plan.planes_de_mejora.filter(p => p.estado_aprobacion === 'validado').length;

                    resumenHTML = `
                    <div class="mb-2 px-2">
                        <div class="d-flex justify-content-between align-items-center mb-1" style="font-size: 0.85rem;">
                            <span><i class="fas fa-clock text-warning mr-1"></i>En espera:</span>
                            <span class="badge badge-warning">${pendientes}/${totalPlanes}</span>
                        </div>
                        <div class="d-flex justify-content-between align-items-center mb-1" style="font-size: 0.85rem;">
                            <span><i class="fas fa-times-circle text-danger mr-1"></i>No validados:</span>
                            <span class="badge badge-danger">${noValidados}/${totalPlanes}</span>
                        </div>
                        <div class="d-flex justify-content-between align-items-center" style="font-size: 0.85rem;">
                            <span><i class="fas fa-check-circle text-success mr-1"></i>Validados:</span>
                            <span class="badge badge-success">${validados}/${totalPlanes}</span>
                        </div>
                    </div>
                `;
                }
                ...
                <div class="mb-4 col-md-3">
                    <div class="card employee-card h-100">
                        <div class="pt-3 text-center card-body">
                            ...
                            <div class="mb-2 badge ${estadoBadge}">Fase ${plan.plan_de_mejora.primera_fase_activa ? 'de registro' : 'de cumplimiento'}</div>
