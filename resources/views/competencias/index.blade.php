        ...
        <div class="modal-dialog modal-xl" role="document">
            <div class="modal-content">
                <div class="modal-header bg-vanguard">
                    <h5 class="text-white modal-title h5" id="editModalLabel">Editar Capacitación</h5>
    ...
    <script type="text/javascript">
        document.addEventListener('DOMContentLoaded', function() {
            var table = new Tabulator("#example-table", {
                ajaxURL: "{{ route('competencias.data') }}",
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        formatter: function(cell, formatterParams) {
                            return "@can('editar-competencias')<button class='text-white btn btn-sm btn-warning edit-button' data-id='" +
                                cell.getRow().getData().id +
                                "'><i class='fas fa-edit'></i></button>@endcan" +
                                "@can('borrar-competencias')<button class='btn btn-sm btn-danger delete-button' data-id='" +
                                cell.getRow().getData().id +
            ...
            $('#saveChanges').on('click', function() {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        var url = (id == '') ? "{{ route('competencias.store') }}" :
                            "{{ route('competencias.update', '') }}/" + id;
                        ...
                        $.ajax({
                            ...
                            error: function(xhr, status, error) {
                                Swal.fire({
                                    icon: 'error',
                                    title: '¡Error!',
                                    text: 'No se pudo actualizar la Capacitación.',
                                });
                            }
                        });
                    }
            ...
            $("#example-table").on("click", ".delete-button", function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('competencias.destroy', '') }}/" + id,
                            type: 'DELETE',
                            data: {
                                _token: '{{ csrf_token() }}'
                            },
